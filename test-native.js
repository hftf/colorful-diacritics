...
function cluster(word, lang, positions) {
    ...
    while (split.length) {
        ...
        if (next === true || next === pos) {
            clustered += '<span class="letter">' + untittled + '</span><span class="diacritic">' + vowels + '</span>';
            if (next !== true)
...
for (var i = 0, l = words.length; i < l; i ++) {
    ...
    word.innerHTML += '<br />' + cluster(word.innerHTML.trim(spaces), findlang(word), positions);
    word.className += " word";
}
